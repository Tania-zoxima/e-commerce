...
class Pship extends Component {
  ...
  searchKeyValueInList(list) {
    let result = "";
    result = list.filter((obj) => obj.zx_orderstatus == "Partially Shipped");
    // console.log(result);
    return result;
  }
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <DetailCard
                  ...
                  onPress={() =>
                    NavigationService.navigate("OrderSecondaryDetails", {
                      data: item,
                    })
                  }
                  heading={item.zx_recordid ? "SOR-" + item.zx_recordid : "NA"}
                  headstyle={
...
const mapStateToProps = (state) => {
  return {
    ...
    count: state.orders.orderCount.count,
  };
};
...
export default connect(mapStateToProps, mapDispatchToProps)(Pship);
