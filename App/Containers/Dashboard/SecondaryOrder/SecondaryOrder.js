...
class SecondaryOrder extends Component {
      ...
      visibleNode = (
        <FlatList
          ...
          renderItem={({ item }) => (
            <DetailCard
              dark={false}
              onPress={() => NavigationService.navigate("SecondaryOrderDetails",{record: item})}
              style={{
              ...
              heading={"SOR-"+item.zx_recordid}
              headstyle={Styles.head}
              date={HelperService.getCurrentDate1(
                item.zx_orderdate
              )}
              datestyle={Styles.date}
              month={HelperService.getMonthMappingName(HelperService.getCurrentMonth(
                item.zx_orderdate
              ))}
              ...
              content={[
                ...
                <DetailCardStrip
                  label={"Amount"}
                  value={"₹"+HelperService.FixedDecimalValue(item.zx_totalamountoforder)}
                  labelStyle={Styles.label}
                  valueStyle={Styles.value}
                />,
              ]}
  ...
  render() {
    const{code,secondarySummary}=this.props
    return (
      <View style={Styles.mainContainer}>
        <Card style={code.zx_brandgroupcode == "1"?Styles.cardstyle:Styles.cardstyleBlue}>
          ...
          <Text style={Styles.title}>
            ...
            <Text style={Styles.titleText}>{secondarySummary && !_.isEmpty(secondarySummary)
                    ? secondarySummary.total_orders
                    : "0"}</Text>
          </Text>
        </Card>
        {this.getDataNode()}
...
const mapStateToProps = (state) => ({
  ...
  data: state.dashboard.data.filterOrder.data && state.dashboard.data.filterOrder.data.secondary_orders.orders,
