...
class ReslovedScreen extends Component {
  ...
  componentDidMount() {
    ...
    this.state.timer = setTimeout(
      () => this.props.getAllLocation({ token }),
      1000
    );
    this.state.timer = setTimeout(
      () => this.props.getNatureCode({ token }),
      2000
    );
  }
  ...
  searchKeyValueInList(list) {
    let result = "";
    result = list.filter((obj) => obj.zx_complaintstatus == "Resolved");
    //  console.log(result);
    return result;
  }
      ...
      if (filteredSitesList.length) {
        visibleNode = (
          <FlatList
            ...
            contentContainerStyle={{ paddingBottom: hp("9%"), paddingTop: 10 }}
            ...
            renderItem={({ item }) => (
              <DetailCard
                ...
                heading={"CI-"+item.zx_recordid}
                headstyle={Style.head}
                date={
                  HelperService.getCurrentDate1(item.zx_dateofregistration) +
                  "\n" +
                  HelperService.getMonthMappingName(
                    HelperService.getCurrentMonth(item.zx_dateofregistration)
                  )
                }
...
export default connect(mapStateToProps, mapDispatchToProps)(ReslovedScreen);
