...
export class Open extends Component {
  ...
  searchKeyValueInList(list) {
    let result = "";
    result = list.filter((obj) => obj.zx_projectstatus == "Open");
    // console.log(result);
    return result;
  }
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <DisplayCard
                  ...
                  // icon1={"create"}
                  // iconStyle={{ marginLeft: wp("10%"), fontSize: 25 }}
                  // locationIcon={{
                  //   width: wp("13%"),
                  //   marginLeft: wp("54%"),
                  //   // marginTop: hp("8%"),
                  //   // marginBottom: hp("4%"),
                  // }}
                  // onPresslocation={() => this.onSelectProject(item)}
                  content={[
                    <DisplayCardStrip
                      stylettl={Styles.ttl}
  ...
  render() {
    // console.log("aabbcccccolorrr", HelperService.getColor());
